...
class Dataset(object):
    def __init__(self, file_name, max_bucket_num=80, word_num_one_batch=5000, sent_num_one_batch=200,
        ...
        if "bc" in self._file_name_short:
            self._domain_id=1
            print("self._domain_id",self._domain_id)
        elif "comment" in self._file_name_short:
            self._domain_id=2
            print("self._domain_id",self._domain_id)
        elif "content" in self._file_name_short:
            self._domain_id=2
            print("self._domain_id",self._domain_id)
            ...
            for line in f:
                ...
                if len(line) == 0:
                    ...
                    if length >= min_len and (max_len < 0 or length <= max_len):
                        self._instances.append(Instance(len(self._instances), lines, self._domain_id))#yli adds self._domain_id
